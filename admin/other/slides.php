...
<body>
          ...
          <table class="table table-striped table-bordered table-hover">
            ...
            <?php while($row = mysql_fetch_assoc($res)):?>
              <tr>
                ...
                <?php if($row['pic_state']=='不显示'):?>
                 <a href="javascript:;" data="<?=$row['pic_id']?>" class="picbtn btn btn-info btn-xs">显示</a>
               <?php else:?>
                <a href="javascript:;"  data="<?=$row['pic_id']?>" class="picbtn btn btn-warning btn-xs">不显示</a>
              <?php endif;?>
  ...
  <script type="text/javascript">
      $(".picbtn").click(function(){
          ...
          var name=$(this).html();
          var _this=$(this);
          $.post("modify_pic.php",{id:id,name:name},function(msg){
            // alert(msg);
            if(msg==1){
                if(name == '显示'){
                    _this.parent().parent().find('.state').html('显示');
                    _this.removeClass('btn-info').addClass('btn-warning').html('不显示');
                }else{
                  _this.parent().parent().find('.state').html('不显示');
